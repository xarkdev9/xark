...
- src/components/os/ChatInput.tsx — Shared input component, always visible across all Space views. Auto-expanding textarea (text.body, 0.75rem) grows upward to ~6 lines max (120px). Enter sends, Shift+Enter newline. Fixed action row below textarea: "attach" (file picker) · "camera" (capture) · "mic" (voice) as text.recency floating labels at textColor(0.3), right-aligned. Actions never move — textarea grows above them. Top ambient accent line (content boundary), bottom accent underline. Controlled by Space page props (input/onInputChange/onSend/isThinking/onAttach/onCamera).
