...
- src/components/os/ChatInput.tsx — Three-element layout. TEXTAREA + MIC: fixed at layout.inputBottom, auto-expanding textarea (text.body) + mic icon (SVG, 14px) in the input row. Top ambient line + bottom accent underline, solid void bg. ATTACH ICON: 16px paperclip SVG, fixed left of ControlCaret dot at caretBottom. CAMERA ICON: 16px camera SVG, fixed right of dot at caretBottom. Icons at colors.white opacity 0.5, thin 1.5px stroke. Mic: tap=dictate, long-press=@xark mode. Void fill between textarea and bottom edge.
