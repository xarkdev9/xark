...
- src/components/os/ChatInput.tsx — Two-zone layout. TEXTAREA ZONE: fixed at layout.inputBottom, auto-expanding textarea (text.body) grows upward to ~6 lines (120px), top ambient line + bottom accent underline, solid void bg. ACTION ORBIT: centered at layout.caretBottom, flanks the ControlCaret dot — "attach" · "camera" · [24px dot gap] · "mic" as text.subtitle at textColor(0.3). Centered via transform translate(-50%). Void fill covers space between textarea and actions. Mic: tap=dictate, long-press=@xark mode. Controlled by Space page props.
