...
Your decision engine and AI (@xark) are your moat. They must run fast, accurately, and cost-efficiently.
...
[ ] ALGO-03: Streaming AI Responses (SSE). Do not hold HTTP requests open for 50 seconds while Apify/Gemini run. Implement Server-Sent Events (SSE) to stream the @xark AI text chunks directly to the Flutter client, showing real-time "typing".

[ ] ALGO-04: Async AI Worker Queue. Move the @xark AI orchestrator off the synchronous hot-path. The API should return 202 Accepted instantly, push the task to Temporal.io or Inngest, and let background workers handle the heavy scraping.
...
[ ] ALGO-06: Taste Graph Mathematical Intersection. Enhance the AI prompt generation. When @xark is queried, mathematically intersect the structured JSON taste profiles of only the active space members, applying strict user_constraints filters before hitting the LLM.
