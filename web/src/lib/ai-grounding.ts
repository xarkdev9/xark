...
function getContextualExamples(spaceTitle?: string): [string, string] {
  ...
  if (/trip|travel|vacation|getaway|visit|weekend/.test(t))
    return ["@hello find hotels near the beach", "@hello add dates aug 15–25"];

  if (/dinner|food|eat|restaurant|lunch|brunch/.test(t))
    return ["@hello find a good spot nearby", "@hello compare top 3 options"];

  if (/party|birthday|celebration|event|wedding/.test(t))
    return ["@hello find a venue for us", "@hello add a budget"];

  if (/buy|purchase|shop|gift/.test(t))
    return ["@hello find the best option", "@hello compare a few choices"];

  // Generic — works for any planning context
  return ["@hello find a few options", "@hello add an idea to the list"];
}
...
// Prevents @hello from suggesting a "Four Seasons" if the group locked the "Hilton."
...
// Produces the dynamic prompt prefix for @hello with weight rules.
