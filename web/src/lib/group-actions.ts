...
// Navigation commands removed — users swipe to switch views. Redundant as @hello commands.

// ── Date commands — mutate via /api/local-action ──
const DATE_COMMANDS: LocalCommand[] = [
  {
    pattern: /@hello\s+(?:set|change|update|modify)\s+(?:trip\s+)?dates?\s+to\s+(.+)/i,
    execute: (match, ctx) => {
...
const RENAME_COMMANDS: LocalCommand[] = [
  {
    pattern: /@hello\s+rename\s+(?:space|this|it|group)\s+to\s+(.+)/i,
    execute: (match, ctx) => {
...
const STATE_QUERY_COMMANDS: LocalCommand[] = [
  {
    pattern: /@hello\s+(?:what(?:'s| is) the )?status/i,
    execute: (_match, ctx) => {
  ...
  {
    pattern: /@hello\s+who\s+hasn(?:'t|t)\s+voted/i,
    execute: (_match, ctx) => {
...
/**
 * Try to handle an @hello message locally.
...
export function tryLocalAgent(
  ...
  if (!text.toLowerCase().includes("@hello")) return null;
