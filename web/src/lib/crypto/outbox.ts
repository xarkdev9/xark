...
export interface OutboxEntry {
  ...
  createdAt: number;
  attempts: number;
  /** Piggybacked Sender Key distribution ciphertexts for atomic retry */
  distributionCiphertexts?: Array<{
    id: string;
    recipient_id: string;
    ...
    try {
      const res = await fetch('/api/message', {
        ...
        body: JSON.stringify({
          ...
          ...(entry.distributionCiphertexts?.length
            ? { distribution_ciphertexts: entry.distributionCiphertexts }
            : {}),
        }),
      });
