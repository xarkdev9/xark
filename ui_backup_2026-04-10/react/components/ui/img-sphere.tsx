...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const renderImageNode = useCallback((image: ImageData, index: number) => {
    ...
    // Calculate thick white border scaling dynamically so it looks perfect
    const borderThickness = Math.max(2, imageSize * 0.04);
    ...
    return (
      ...
      >
        <div 
          className="relative w-full h-full rounded-full overflow-hidden bg-white shadow-[0_15px_35px_-5px_rgba(0,0,0,0.2)] hover:shadow-[0_20px_45px_-5px_rgba(0,0,0,0.3)] transition-shadow duration-300" 
  ...
  const renderSpotlightModal = () => {
    ...
    return (
      ...
      >
        <div
          className="bg-white rounded-2xl max-w-sm w-full overflow-hidden shadow-2xl border border-gray-100"
        ...
        >
          <div className="relative aspect-square">
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-4 right-4 w-10 h-10 bg-black/40 backdrop-blur rounded-full text-white flex items-center justify-center hover:bg-black/60 transition-all cursor-pointer"
            >
          ...
          {(selectedImage.title || selectedImage.description) && (
            <div className="p-6">
              ...
              {selectedImage.description && <p className="text-gray-500 font-light">{selectedImage.description}</p>}
            </div>
          )}
        </div>
      </div>
  ...
  if (!isMounted) return <div style={{ width: containerSize, height: containerSize }} className="animate-pulse bg-gray-50 rounded-full" />;
  if (!images.length) return <div style={{ width: containerSize, height: containerSize }} className="bg-gray-50 flex items-center justify-center rounded-full"><p className="text-gray-400">No images</p></div>;
