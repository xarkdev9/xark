...
export function ConsensusMark({
  ...
  // ── SEEKING (0-30%): Amber dashed ring, tween pulse ──
  if (state === "seeking") {
    return (
      <motion.svg
        width={size}
  ...
  // ── STEADY (31-80%): Amber ring + breathing accent dot ──
  if (state === "steady") {
    return (
  ...
  // ── IGNITED (80%+): Gold ring + radial flare particles ──
  const flareCount = 6;
  const flares = Array.from({ length: flareCount }, (_, i) => {
    const angle = (i / flareCount) * Math.PI * 2;
    const flareR = r + 4;
    return {
      x: cx + Math.cos(angle) * flareR,
      y: cy + Math.sin(angle) * flareR,
      delay: i * 0.15,
    };
  });
