...
class _PlansViewState extends ConsumerState<PlansView> {
  ...
  Widget build(BuildContext context) {
    ...
    final groupName = widget.groupId;

    final plansAsync = ref.watch(decryptedPlansProvider(widget.groupId));

    return plansAsync.when(
      loading: () => const Center(child: CircularProgressIndicator(color: HelloColors.accent)),
      error: (e, st) => Center(child: Text('Error decrypting plans: $e', style: HelloTypography.hint)),
      data: (allItems) {
        if (allItems.isEmpty) {
          return const Center(
        ...
        late final Map<String, List<DecryptedItemPayload>> eventItemsMap;

        if (hasDelimiter) {
          // Multi-event: parse "Event|Category"
          eventItemsMap = <String, List<DecryptedItemPayload>>{};
        ...
        List<DecryptedItemPayload> catItems;
  ...
  Widget _buildHero(
    ...
    String eventName,
    List<DecryptedItemPayload> catItems,
    List<DecryptedItemPayload> allEventItems,
    ...
    if (category == 'Recommendations') {
      final sorted = List<DecryptedItemPayload>.from(allEventItems)
        ..sort((a, b) => b.obliviousItem.agreementScore.compareTo(a.obliviousItem.agreementScore));
...
class _EventRail extends StatelessWidget {
  ...
  final Map<String, List<DecryptedItemPayload>> eventItems;
  ...
  Widget build(BuildContext context) {
    ...
    final sorted = List<DecryptedItemPayload>.from(items)
      ..sort((a, b) {
        ...
        if (!a.isLocked && b.isLocked) return -1;
        return b.obliviousItem.agreementScore.compareTo(a.obliviousItem.agreementScore);
      });
      ...
      child: Column(
        ...
        children: [
          ...
          ...sorted.map((item) => Padding(
            ...
            child: item.obliviousItem.isLocked
...
class _FloatingActiveCard extends StatelessWidget {
  final DecryptedItemPayload item;
  ...
  String _buildSocialText(DecryptedItemPayload item) {
    final proposer = item.proposedBy ?? 'someone';
...
class _FloatingSettledCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final proposer = item.obliviousItem.proposedBy ?? 'someone';
    final hasPhoto = item.photoUrl != null && item.photoUrl!.isNotEmpty;
