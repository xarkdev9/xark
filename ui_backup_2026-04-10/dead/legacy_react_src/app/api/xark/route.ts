...
export async function POST(req: NextRequest) {
  ...
  if (!checkRateLimit(`hello:${auth.userId}`, 10)) {
    return NextResponse.json({
  ...
  if (isFollowUp && !checkRateLimit(`hello-followup:${spaceId}`, 1, 300_000)) {
  ...
  const webhookUrl = `${baseUrl}/api/hello/webhook?spaceId=${encodeURIComponent(spaceId)}&msgId=${encodeURIComponent(helloMsgId)}&label=${encodeURIComponent(queryTextForLabel)}`;
