// hello OS v2.0 — E2EE Message Endpoint
...
export async function POST(req: NextRequest) {
  try {
    ...
    const msgId = (bodyId && typeof bodyId === 'string') ? bodyId : `msg_${crypto.randomUUID()}`;
    ...
    // PRIORITY 3 FIX: All distribution rows are fully awaited and committed BEFORE
    // the response is returned. The client only broadcasts after receiving 200 OK
    // with distribution_written: true, eliminating the Realtime race condition.
    ...
    return NextResponse.json({ messageId: msgId, distribution_written: distributionWritten });
