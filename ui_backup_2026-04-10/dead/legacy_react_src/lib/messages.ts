...
export async function fetchMessages(
  ...
  if (error) {
    console.error("[hello] fetchMessages failed:", error.message, { spaceId });
    return [];
  }
  if (!data) return [];
...
export async function saveMessage(msg: {
  ...
  if (error) {
    console.error("[hello] saveMessage failed:", error.message, { userId: msg.userId, spaceId: msg.spaceId, hasJWT });
    throw error;
  }
}
