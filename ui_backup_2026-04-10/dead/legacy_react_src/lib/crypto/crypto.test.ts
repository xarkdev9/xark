...
describe("Cryptographic Primitives", () => {
  ...
  it("derives keys with HKDF", () => {
    ...
    const key1 = hkdf(ikm, salt, "helloE2EE-test", 32);
    const key2 = hkdf(ikm, salt, "helloE2EE-test", 32);
  ...
  it("derives different keys for different info strings", () => {
    const ikm = randomBytes(32);
    const salt = randomBytes(32);
    const key1 = hkdf(ikm, salt, "helloE2EE-chat", 32);
    const key2 = hkdf(ikm, salt, "helloE2EE-backup", 32);
