...
export async function registerKeys(): Promise<{
  ...
  // 4. Upload public keys to server FIRST — before saving locally.
  // This prevents local/server state divergence: if upload fails,
  // hasRegisteredKeys() returns false → retry on next visit.
  ...
  // 5. Upload OTK public keys via API route
  ...
  // 6. Save private keys locally — ONLY after server confirmed both uploads.
...
export async function onMemberLeave(
  ...
  if (currentKey) {
    ...
    console.log(`[hello-e2ee] Archived old Sender Key for space ${spaceId}`);
  }
  ...
  console.log(`[hello-e2ee] Rotated Sender Key for space ${spaceId} (member ${leftUserId} left)`);
  return newKey;
}
...
export function subscribeToMemberChanges(
  ...
  const channel = supabase
    ...
    .on('postgres_changes', {
      ...
      if (isLeader) {
        console.log(`[hello-e2ee] I am SK rotation leader for ${spaceId}`);
        try {
          const newKey = await onMemberLeave(spaceId, leftUserId);
          await onRotation(newKey);
        } catch (err) {
          console.error(`[hello-e2ee] SK rotation failed for ${spaceId}:`, err);
        }
      } else {
        console.log(`[hello-e2ee] Waiting for leader to rotate SK for ${spaceId}`);
