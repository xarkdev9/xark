...
export async function requestMissingSenderKey(
  ...
  console.log(`[hello-sk-recovery] Requesting SK from ${senderId} for space ${spaceId}`);
...
export async function handleSenderKeyRequest(
  ...
  console.log(`[hello-sk-recovery] Authorized SK request from ${requesterId} for space ${spaceId}`);
  return true;
}
