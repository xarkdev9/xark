...
export interface LinkPreviewPayload {
  ...
  title?: string;
  description?: string;
  /** Firebase URL of encrypted og:image blob (if present) */
  mediaUrl?: string;
  /** AES-256-GCM key for the encrypted og:image */
  aesKeyBase64?: string;
  /** AES-GCM IV for the encrypted og:image */
  ivBase64?: string;
  /** MIME type of the og:image */
  mimeType?: string;
  /** Tiny blurry thumbnail of the og:image */
...
export interface DecryptedMessage {
  ...
  type: 'message' | 'media';
  /** AES-256-GCM key for decrypting the media blob (base64, media messages only) */
  aesKeyBase64?: string;
  /** AES-GCM IV for decrypting the media blob (base64, media messages only) */
  ivBase64?: string;
  /** Original MIME type of the media file (e.g. 'image/jpeg') */
  mimeType?: string;
  /** Tiny Base64 JPEG thumbnail for instant blurry preview (data:image/jpeg;base64,...) */
  inlineThumbnail?: string;
  /** Attached link preview (OG metadata + encrypted preview image) */
