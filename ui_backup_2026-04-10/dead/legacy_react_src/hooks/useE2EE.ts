...
async function requestSKsFromAllSpaces(userId: string, deviceId: number): Promise<void> {
  try {
    ...
    console.log(`[hello-e2ee] Proactive SK recovery: ${spaceIds.length} spaces`);
    ...
    for (const spaceId of spaceIds) {
      ...
      console.log(`[hello-e2ee] Requested SK from ${devices.length} member(s) in ${spaceId.slice(0, 12)}...`);
    }
  } catch (err) {
