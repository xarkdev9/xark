...
/** Parse encrypted image payload from metadata */
...
function parseEncryptedImage(metadata?: Record<string, unknown> | null): EncryptedImagePayload | null {
  if (!metadata?.encrypted_image) return null;
  try {
    const raw = typeof metadata.encrypted_image === 'string'
      ? JSON.parse(metadata.encrypted_image)
      : metadata.encrypted_image;
    if (raw?.mediaUrl && raw?.aesKeyBase64 && raw?.ivBase64 && raw?.mimeType) {
      return raw as EncryptedImagePayload;
    }
  } catch { /* invalid JSON — treat as no encrypted image */ }
  return null;
}
...
export function DecisionCard({
  ...
  return (
    <motion.div
      className={`relative shadow-2xl ${fullFocus ? "" : "flex-shrink-0"}`}
      style={{
        width: fullFocus ? "100%" : "70%",
        maxWidth: fullFocus ? "none" : "260px",
        height: fullFocus ? "100%" : "clamp(340px, 50dvh, 420px)",
        borderRadius: fullFocus ? "20px" : "26px",
        overflow: "hidden",
        scrollSnapAlign: fullFocus ? undefined : "center",
        cursor: onClick || (isCommitted && (bookingUrl || bookingPhone)) ? "pointer" : "default",
        transform: "translateZ(0)",
        border: "1px solid rgba(255,255,255,0.08)",
      }}
      initial={{ opacity: 0, scale: fullFocus ? 1 : 0.92, filter: fullFocus ? "none" : "blur(8px)", y: 0 }}
      animate={{
        opacity: 1,
        scale: 1,
        filter: "blur(0px)",
    ...
    >
      {/* ── Full-bleed photo (plaintext URL or E2EE encrypted blob) ── */}
      {(() => {
        const encPayload = parseEncryptedImage(metadata as Record<string, unknown> | null);
        if (encPayload) {
          // E2EE path: decrypt and render via EncryptedMediaRenderer
          return (
            <EncryptedMediaRenderer
              mediaUrl={encPayload.mediaUrl}
              aesKeyBase64={encPayload.aesKeyBase64}
              ivBase64={encPayload.ivBase64}
              mimeType={encPayload.mimeType}
              inlineThumbnail={encPayload.inlineThumbnail}
              fillContainer
            />
          );
        }
        if (imageUrl && !imgError) {
          // Legacy plaintext URL path
          return (
            <motion.img
              src={imageUrl}
              alt=""
      ...
      <div
        ...
        style={{
          height: "44px",
          padding: "0 12px",
          justifyContent: fullFocus ? "center" : "space-between",
          gap: fullFocus ? "8px" : undefined,
        }}
      >
        {SIGNALS.map((signal) => {
