  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        const req = indexedDB.deleteDatabase('hello-keystore');
      ...
      localStorage.removeItem('hello_store_salt');
    ...
    >
      ...
      <div style={{ marginTop: "24px", display: "flex", flexDirection: "column", gap: "8px" }}>
        ...
        <div className="relative">
          ...
          />
          {/* Save button — visible when name is changed */}
          {editName.trim() && editName.trim() !== userName && (
            <span
              role="button"
              tabIndex={0}
              onClick={handleNameSave}
              className="absolute right-0 top-1/2 -translate-y-1/2 cursor-pointer outline-none"
              style={{ fontSize: "13px", fontWeight: 400, color: colors.accent, letterSpacing: "0.04em" }}
            >
  ...
  const AboutView = (
    ...
    >
      ...
      <span
        role="button"
        tabIndex={0}
        onClick={() => window.open("mailto:feedback@gethello.ai")}
        onKeyDown={(e) => { if (e.key === "Enter") window.open("mailto:feedback@gethello.ai"); }}
        className="cursor-pointer outline-none"
        style={{
          ...text.body,
          display: "inline-block",
