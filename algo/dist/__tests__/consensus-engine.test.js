...
import { ReactionType, BookableItemState, TaskState, EventType, } from "../models/types.js";
...
describe("ConsensusEngine", () => {
    ...
    describe("Full Green-Lock Flow", () => {
        it("completes the full lifecycle: propose → react → sort → lock → transfer", () => {
            ...
            const hilton = engine.proposeItem("trip_sd", "Hilton San Diego", "Downtown, pool, $200/night", "hotel", "apify", 1000);
            const marriott = engine.proposeItem("trip_sd", "Marriott San Diego", "Gaslamp, rooftop bar, $250/night", "hotel", "apify", 1001);
            const hyatt = engine.proposeItem("trip_sd", "Hyatt San Diego", "Harbor view, $180/night", "hotel", "apify", 1002);
            // Verify all proposed
            ...
            engine.react(hilton.id, "alice", ReactionType.Heart, 2000); // +5
            engine.react(hilton.id, "bob", ReactionType.ThumbsUp, 2001); // +1
            engine.react(marriott.id, "alice", ReactionType.ThumbsUp, 2002); // +1
            engine.react(marriott.id, "carol", ReactionType.Heart, 2003); // +5
            engine.react(marriott.id, "dave", ReactionType.Heart, 2004); // +5
            engine.react(hyatt.id, "bob", ReactionType.ThumbsUp, 2005); // +1
            ...
            // Verify items transition to heart_sorted state
            expect(sorted[0].state).toBe(BookableItemState.HeartSorted);
    ...
    describe("AI Grounding", () => {
        it("grounds @xark to locked state — prevents contradicting bookings", () => {
            // Book a hotel
            const hotel = engine.proposeItem("trip_sd", "Hilton San Diego", "Downtown", "hotel", "apify", 1000);
            engine.react(hotel.id, "alice", ReactionType.Heart, 1500);
            ...
            const nobu = engine.proposeItem("trip_sd", "Nobu San Diego", "Japanese cuisine", "restaurant", "apify", 2500);
            engine.react(nobu.id, "bob", ReactionType.Heart, 2600);
            // Assign a task
            ...
            expect(context.lockedDecisions.find((d) => d.type === "locked_booking").title)
    ...
    describe("Heart-Sort Queries", () => {
        it("returns ranked items with agreement scores", () => {
            ...
            engine.react(h.id, "alice", ReactionType.Heart, 1100);
            engine.react(h.id, "bob", ReactionType.Heart, 1200);
            engine.react(h.id, "carol", ReactionType.Heart, 1300);
            engine.react(h.id, "dave", ReactionType.ThumbsUp, 1400);
        ...
        it("excludes locked items from ranked view", () => {
            ...
            engine.react(a.id, "alice", ReactionType.Heart, 1100);
            engine.lock(a.id, {
                type: "confirmation_number",
                value: "X",
                submittedBy: "alice",
    ...
    describe("Reaction Rules", () => {
        it("prevents reactions on locked items", () => {
            ...
            expect(() => engine.react(item.id, "bob", ReactionType.Heart, 3000)).toThrow("locked");
        });
        it("replaces reaction when same user reacts again", () => {
            const item = engine.proposeItem("trip_sd", "Hotel", "", "hotel", "apify", 1000);
            engine.react(item.id, "alice", ReactionType.ThumbsUp, 1100);
            const updated = engine.react(item.id, "alice", ReactionType.Heart, 1200);
            expect(updated.weightedScore).toBe(5); // Heart replaces thumbs up
    ...
    describe("Task Assignment", () => {
        ...
        it("emits events for task operations", () => {
            ...
            const taskEvents = events.filter((e) => e.type === EventType.TaskCreated || e.type === EventType.TaskAssigned);
    ...
    describe("Edge Cases", () => {
        it("throws for non-existent item", () => {
            expect(() => engine.react("nonexistent", "alice", ReactionType.Heart, 1000)).toThrow("not found");
        });
        ...
        it("handles agreement score for empty group", () => {
            ...
            engine.react(item.id, "alice", ReactionType.Heart, 1100);
