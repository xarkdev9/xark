...
describe("ConsensusEngine", () => {
    ...
    describe("Full Green-Lock Flow", () => {
        it("completes the full lifecycle: propose → react → sort → lock → transfer", () => {
            ...
            const hilton = engine.proposeItem("trip_sd", "Hilton San Diego", "nonce", "apify", 1000);
            const marriott = engine.proposeItem("trip_sd", "Marriott San Diego", "nonce", "apify", 1001);
            const hyatt = engine.proposeItem("trip_sd", "Hyatt San Diego", "nonce", "apify", 1002);
            // Verify all proposed
    ...
    describe("AI Grounding", () => {
        it("grounds @hello to locked state — prevents contradicting bookings", () => {
            // Book a hotel
            const hotel = engine.proposeItem("trip_sd", "Hilton San Diego", "nonce", "apify", 1000);
            ...
            const nobu = engine.proposeItem("trip_sd", "Nobu San Diego", "nonce", "apify", 2500);
    ...
    describe("Reaction Rules", () => {
        it("prevents reactions on locked items", () => {
            ...
            expect(() => engine.react(item.id, "bob", ReactionType.LoveIt, 3000)).toThrow("locked");
        });
        it("prevents Not for me on locked items", () => {
            const item = engine.proposeItem("trip_sd", "Hotel", "nonce", "apify", 1000);
            engine.lock(item.id, "X", "nonce", "alice", 2000);
            expect(() => engine.react(item.id, "bob", ReactionType.NotForMe, 3000)).toThrow("locked");
        });
    ...
    describe("Edge Cases", () => {
        it("throws for non-existent item", () => {
            expect(() => engine.react("nonexistent", "alice", ReactionType.LoveIt, 1000)).toThrow("not found");
        });
