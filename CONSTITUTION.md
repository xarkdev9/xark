...
- **ChatInput** (`src/components/os/ChatInput.tsx`): Three-element layout. **Textarea + mic** at `layout.inputBottom`: auto-expanding textarea (`text.body`) + mic icon (14px SVG) inside the input row. Enter sends, Shift+Enter newline. Top ambient accent line. Bottom accent underline. Solid `colors.void` bg. Mic: tap = dictate, long-press 500ms = @xark mode (breathing dot replaces icon when active). **Attach icon** (16px paperclip SVG): fixed left of ControlCaret dot at `layout.caretBottom`. **Camera icon** (16px camera SVG): fixed right of dot. Both at `colors.white` opacity 0.5 → 0.8 on hover. Thin 1.5px stroke. Void fill between textarea and bottom edge.
- **BANNED in ChatInput**: Buttons with backgrounds, text-label actions at dot level (use icons), full-width action bars, actions that shift position.
...
- **Galaxy** (`src/app/galaxy/page.tsx`): The Active Mind — awareness stream home. Awareness stream (src/lib/awareness.ts) shows priority-sorted cross-space events: needs_vote, ignited, proposal, locked, message, joined. Each event = whisper text + space context + recency label. Tap event → navigate to space. Amber swell intensifies when recent activity (<15 min). Mesh Pulse (15s breath). Empty state: "who are you planning with?" at `text.listTitle`. Input at 96px bottom. Manifestation Loop: optimistic `router.push()` via `getOptimisticSpaceId()` → `createSpace()` parallel. ControlCaret global via `layout.tsx`. UserMenu (profile + theme selector) visible on Galaxy only. Spectrum Wash. @xark greeting anchored near input when stream empty.
- **Space** (`src/app/space/[id]/page.tsx`): Individual space with `discuss` / `decide` / `itinerary` / `memories` / `share` as floating text (`text.label`, outline-none). discuss + decide = view toggle. share = action (navigator.share on mobile, clipboard copy on desktop with "link copied" whisper for 2s). Active = `colors.cyan` at 0.9. Inactive = `colors.white` at 0.4. Chat state (messages, input, isThinking, sendMessage) owned HERE — persists across view switches. ChatInput always visible. XarkChat is display-only (receives messages as props). Chat state (messages, input, isThinking) lifted to Space page — persists across view switches. `ChatInput` always visible across all views. `discuss` renders `XarkChat` (display-only, receives messages as props). `decide` renders `PossibilityHorizon`. Fixed header (`text.spaceTitle`) with gradient fade + ambient accent line. Demo data fallback. ControlCaret global via `layout.tsx`. No UserMenu on space pages.
...
- **Decision Cards**: 170×220px portrait visual + reaction row below. No border-radius. Image or gradient placeholder (stronger: `amber 0.2 → accent 0.12 → void 0.4`). Strong bottom vignette (0.98 → 0.6 at 45%). Amber wash from `amberWash(weightedScore)`. **Consensus Hero**: 1.5rem percentage number in consensus color (amber seeking, cyan steady, gold ignited at 0.95 opacity) + 2px progress bar. Title (`text.body`, 0.9). Price (`text.recency`, 0.4). **Reactions BELOW card** on void background for sunlight readability: "love" / "okay" / "pass" in `text.body` (0.75rem) at opacity 0.6, each in its signal color. Active reaction at 1.0 opacity.
...
- **BANNED**: Cards with border-radius, rounded corners, `ConsensusMark` animated SVG on cards, conviction strips, buttons with borders/backgrounds.
