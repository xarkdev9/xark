...
- **ChatInput** (`src/components/os/ChatInput.tsx`): Two-zone layout. **Textarea zone** at `layout.inputBottom`: auto-expanding textarea (`text.body`) grows upward, max ~6 lines (120px). Enter sends, Shift+Enter newline. Top ambient accent line (content boundary). Bottom accent underline. Solid `colors.void` bg. Void fill covers gap between textarea and actions. **Action orbit** centered at `layout.caretBottom`: flanks the ControlCaret breathing dot — "attach" · "camera" · [24px dot gap] · "mic" as `text.subtitle` at `textColor(0.3)`. Centered via `transform: translate(-50%, 50%)`. Mic: tap = dictate, long-press 500ms = @xark mode.
- **BANNED in ChatInput**: Icons, buttons with backgrounds, actions inside the textarea zone, full-width action bars, actions that shift position.
