...
- **Decision Cards**: 200×260px. No border-radius. Image or gradient placeholder (`amber → accent`). Bottom vignette (stronger: 0.5 at 50%). Amber wash from `amberWash(weightedScore)`. **Consensus Hero**: 1.5rem percentage number in consensus color (amber seeking, cyan steady, gold ignited at 0.95 opacity) + 2px progress bar filling with agreement. Title (`text.body`, 0.9). Price (`text.recency`, 0.4). Compact reaction signals: "love" / "okay" / "pass" in `text.recency` (single line, no uppercase).
